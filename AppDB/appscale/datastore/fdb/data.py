...
class DataManager(object):
  ...
  def _last_chunk(self, tr, path_subspace, read_vs=None):
    ...
    if read_vs is not None:
      ...
      logger.debug('struct format str: {}'.format(read_vs._STRUCT_FORMAT_STRING))
      logger.debug('tr_version: {}'.format(read_vs.tr_version))
      logger.debug('is complete: {}'.format(read_vs.is_complete()))
      logger.debug('unset tr version: {}'.format(read_vs._UNSET_TR_VERSION))
      logger.debug('user_version: {}'.format(read_vs.user_version))
